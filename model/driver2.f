...
c 
	  ...
	  call energy_balance_climate_model(seasons, snowball, tend, dt, rot, a, ecc, peri, obl, cloudir, ocean, igeog, groundalb, relsolcon, landsnowfrac, fcloud, yrstep,resfile, noisevar, stochastic, soladj, d0)
	  contains
	  
	  
	  subroutine energy_balance_climate_model(seasons, snowball, tend, dt, rot, a, ecc, peri, obl, cloudir, ocean, igeog, groundalb, relsolcon, landsnowfrac, fcloud, yrstep,resfile, noisevar, stochastic, soladj, d0)
c--------------------------------------------------------------------c
...
c----------------------------------------------------------------------c
      ...
      real*8  lat,latangle,mu,ir,msun,landalb,icealb,irsum,irave,mp,
...
c      INTEGER namelistid
c      OPEN( namelistid, FILE='./input.nml', DELIM='APOSTROPHE' )
c

c  INITIALIZE VARIABLES
      ann_tempave = 10.         !just some constant greater than zero
c      seasons = .true.     !if .true. do seasons: otherwise do mean-annual
      last = .false.       !should always be .false.
c      snowball = .false.   !if .true. then start as an ice-ball
c      stochastic = .false. !if .true. then include stochastic perturbation
c      soladj = .false.     !if .true. then include solar forcing adjustment
c      resfile = 0          !start from previous (1), present Earth (2) hothouse (3), large ice (4)
c      tend = 7.e11          !calculation length (sec)
c     dt = 8.64e4            !time step (sec)
      rot0 = 7.27e-5       !Earth's present rotation rate (rad/sec)
c     rot = rot0           
      a0 = 1.49597892E13   !Earth's orbit semi-major axis (cm)
c     a = a0               !semi-major axis for this run
c     ecc = 0.016708617   !orbit eccentricity
c     peri = 76.25     !longitude of perihelion wrt vernal equinox (degrees)
c     obl = 23.45               !obliquity (degrees)
c     cloudir = -9.5   !correction to outgoing infrared for clouds (W/m^-2)
      pco20 = 3.3e-4   !present atmospheric CO2 concentration (bars)
      pco2 = pco20              !initial co2 concentration for this run (bars)
c     ocean = 0.7      !planet fraction covered by water
c     igeog = 1        !geography ('1'=present,'2'=polar,'3'=equatorial,
     &                 ! '4'=100% water,'5'=100% land)
c     groundalb = 0.20  !base land surface albedo
      !groundalb = 0.25
c     relsolcon = 1.0  !relative solar constant (1.0 for present Earth) 
      alpha = -0.078            !alpha: cloud albedo = alpha + beta*zrad
      beta = 0.65      !beta
      cw = 2.1e8       !heat capacity over ocean
      cl = 5.25e6      !heat capacity over land
c     d0 = 0.58        !thermal diffusion coefficient
...
c     landsnowfrac = 1.0 !snowfall fraction on land
c     fcloud = 0.5     !fractional cloud cover
c     noisevar = 0.15  ! noise variance (K^2 per year)
